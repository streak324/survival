...
b32 ResolveAnyCollision(i32 Thing1, i32 Thing2, f32 FrameTime)
{
	...
	//the ray is thing 1 relative to thing 2
	v2 RayPos = v2{GameThings.X[Thing1] - GameThings.X[Thing2], GameThings.Y[Thing1] - GameThings.Y[Thing2]};
	v2 RayDir = v2{GameThings.VelocityX[Thing1] - GameThings.VelocityX[Thing2], GameThings.VelocityY[Thing1] - GameThings.VelocityY[Thing2]};
	
	f32 RayMag = Len(RayDir - RayPos);
	f32 RayMagFrame = RayMag * FrameTime;

	f32 OriginToRayDist = Len(RayPos);
	b32 RayIsInCircle = OriginToRayDist < SumRadius;

	if(DebuggingSwept && RayMagFrame > CollisionEpsilon) //ray-circle intersection
	{
		v2 RayDirFrame = RayDir * FrameTime;
		f32 DotDir = Dot(RayDirFrame, RayPos);
		f32 InsideSqrt = DotDir * DotDir - (Dot(RayPos, RayPos) - SumRadius * SumRadius);
		if(InsideSqrt < 0 || (RayIsInCircle == 0 && DotDir >= 0))
		{
			return 0; 
		}
		f32 SqrtValue = Sqrt(InsideSqrt);
		f32 ResolveScalar1 = -DotDir - SqrtValue;
		f32 ResolveScalar2 = -DotDir + SqrtValue;
		v2 Resolve1 = RayDir * ResolveScalar1;
		v2 Resolve2 = RayDir * ResolveScalar2;
		v2 Resolve = Resolve1;
		f32 ResolveScalar = ResolveScalar1;
		f32 DotResolve = Dot(Resolve, RayDir);
		if(DotResolve < 0)
		{
			Resolve = Resolve2;
		...
		if(0 && ResolveScalar < 1)
		{
			f32 LeftoverVelRatio = (1 - Abs(ResolveScalar));
			v2 Vel1 = v2{GameThings.VelocityX[Thing1], GameThings.VelocityY[Thing1]} * LeftoverVelRatio;
			v2 Vel2 = v2{GameThings.VelocityX[Thing2], GameThings.VelocityY[Thing2]} * LeftoverVelRatio;
	...
	else //point-circle intersection
	{
		...
		v2 RayUnit = RayPos / OriginToRayDist;
		f32 MinSeparationDist =  SumRadius - OriginToRayDist + CollisionEpsilon;
		v2 Separation = RayPos * MinSeparationDist;
